...
func (e Entry) callFirst(p *Parser, tok *lexer.Token) (Node, error) {
	if e.First == nil {
		return nil, UnexpectedToken(tok)
	}

	return e.First(p, e.Power, tok)
}

// callNext is a helper to call the declared ExprNext function.
func (e Entry) callNext(p *Parser, l Node, tok *lexer.Token) (Node, error) {
	if e.Next == nil {
		return nil, UnexpectedToken(tok)
	}

	return e.Next(p, e.Power, l, tok)
}

// callStmt is a helper to call the declared Statement function.
func (e Entry) callStmt(p *Parser, tok *lexer.Token) (Node, error) {
	if e.Stmt == nil {
		return nil, UnexpectedToken(tok)
	}
